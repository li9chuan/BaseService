...
const uint32 MsgProtoTypeSize = 1;
...
struct TReceivedMessage
{
	...
	enum TEventType { User = 'U', RemoveClient = 'D', Command = 'C' };
	...
	const uint8			*userDataR() const				{ return &*_Data.begin() + MsgHeaderSize + MsgProtoTypeSize; }

	/// Return the size of user data
	uint32				userSize()						{ return (uint32)_Data.size() - MsgHeaderSize - MsgProtoTypeSize; }

	/// Return the data vector (event type header byte + user data)
	std::vector<uint8>&	data()							{ return _Data; }

    bool   isProtoCMD()         { return _Data[1] & PROTO_CMD;      }
    bool   isProtoNeedACK()     { return _Data[1] & PROTO_NEED_ACK; }
